...
export function CandidatesPage({ apiFetch, showToast, statusConfig, globalSearchTerm, refreshTrigger, setActivePage, filters, setFilters }) {
                        ...
                        <tbody>
                            {candidates.length > 0 ? candidates.map(c => {
                                ...
                                <tr key={c.id} className={`border-b border-slate-200 ${selectedIds.has(c.id) ? 'bg-primary-light' : 'hover:bg-slate-50'}`}>
                                    ...
                                    {/* === THIS IS THE FIX: JOB TITLE STYLING === */}
